...
// Delegates to the shared helper so we don't duplicate logic.
// Kept the same name/signature so nothing else in this file breaks.
const { notifyUser } = require('./notificationController');

const createNotification = async (userId, title, message, data = {}) => {
  try {
    return await notifyUser(userId, { title, message, data });
  } catch (err) {
...
exports.assignAgent = async (req, res) => {
  try {
    ...
    // ── Notify the agent that they've been assigned ──────────────
    await createNotification(
      agent_id,
      `New delivery assigned`,
      `You have been assigned to deliver order #${order.id}.`,
      { orderId: order.id, kind: 'delivery_assigned' }
    );
